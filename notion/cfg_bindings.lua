...
defbindings("WScreen", {
  ...
  bdoc("Create a new workspace of chosen default type.", "create ws"),
  kpress(META.."F9", "ioncore.create_ws(_)"),

  bdoc("Display the main menu.", "main menu"),
  kpress(META.."M", "mod_menu.menu(_, _sub, 'mainmenu', {big=true})"),
  kpress(ALTMETA.."M", "mod_query.query_menu(_, _sub, 'mainmenu', 'Main menu:')"),
  -- mpress("Button3", "mod_menu.pmenu(_, _sub, 'mainmenu')"),

  bdoc("Display the window list menu.", "winlist"),
  kpress(META.."Q", "mod_menu.pmenu(_, _sub, 'windowlist')"),
...
defbindings("WMPlex.toplevel", {
  ...
  bdoc("Raise Master Volume all tags.", "-tags"),
  kpress("XF86AudioRaiseVolume","ioncore.exec_on(_, 'amixer set Master 2%+')"),
  bdoc("Lower Master Volume all tags.", "-tags"),
  ...
  kpress(ALTMETA.."X", "ioncore.clear_tags()"),
  ...
  kpress(META.."C", "mod_query.exec_on_merr(_, XTERM or 'xterm')"),
  kpress(META.."T", "mod_query.exec_on_merr(_, XTERM or 'xterm')"),

  bdoc("Query for command line to execute.", "run"),
  kpress(META.."E", "mod_query.query_exec(_)"),

  bdoc("Query for Lua code to execute.", "lua"),
  kpress(ALTMETA.."E", "mod_query.query_lua(_)"),

  bdoc("Query for host to connect to with SSH.", "ssh"),
  kpress(ALTMETA.."C", "mod_query.query_ssh(_, ':ssh')"),
  ...
  kpress(ALTMETA.."M", "mod_menu.menu(_, _sub, 'ctxmenu')"),
...
defbindings("WFrame", {
  ...
  kpress(META.."J", "WFrame.maximize_horiz(_)"),
  bdoc("Maximize the frame vertically.", "vmax"),
  kpress(ALTMETA.."J", "WFrame.maximize_vert(_)"),

  bdoc("Display context menu.", "context menu"),
  mpress("Button3", "mod_menu.pmenu(_, _sub, 'ctxmenu')"),

  bdoc("Begin move/resize mode.", "resize"),
  kpress(META.."R", "WFrame.begin_kbresize(_)"),

  bdoc("Switch the frame to display the object indicated by the tab.", "switch to frame"),
  mclick("Button1@tab", "WFrame.p_switch_tab(_)"),
  mclick("Button2@tab", "WFrame.p_switch_tab(_)"),

  bdoc("Resize the frame.", "reszie frame"),
  mdrag("Button1@border", "WFrame.p_resize(_)"),
  mdrag(META.."Button3", "WFrame.p_resize(_)"),

  bdoc("Move the frame.", "move frame"),
  mdrag(META.."Button1", "WFrame.p_move(_)"),

  bdoc("Move objects between frames by dragging and dropping the tab.", "drag frame"),
...
defbindings("WFrame.toplevel", {
  ...
  kpress(ALTMETA.."V", "mod_query.query_attachclient(_)"),

  bdoc("Switch to tab 0 in this frame.", "tab 0"),
  kpress(META.."F1", "WFrame.switch_nth(_, 0)"),
  bdoc("Switch to tab 1 in this frame.", "tab 1"),
  kpress(META.."F2", "WFrame.switch_nth(_, 1)"),
  bdoc("Switch to tab 2 in this frame.", "tab 2"),
  kpress(META.."F3", "WFrame.switch_nth(_, 2)"),
  bdoc("Switch to tab 3 in this frame.", "tab 3"),
  kpress(META.."F4", "WFrame.switch_nth(_, 3)"),
  bdoc("Switch to tab 4 in this frame.", "tab 4"),
  kpress(META.."F5", "WFrame.switch_nth(_, 4)"),
  bdoc("Switch to tab 5 in this frame.", "tab 5"),
  kpress(META.."F6", "WFrame.switch_nth(_, 5)"),
...
defbindings("WFrame.floating", {
  bdoc("Toggle shade mode", "troggle shade mode"),
  mdblclick("Button1@tab", "WFrame.set_shaded(_, 'toggle')"),

  bdoc("Raise the frame.", "raise frame"),
  ...
  bdoc("Lower the frame.", "lower frame"),
...
defbindings("WMoveresMode", {
  ...
  kpress("W",     "WMoveresMode.resize(_, 1, 0, 0, 0)"),
  kpress("S",     "WMoveresMode.resize(_, 0, 1, 0, 0)"),
  kpress("A",     "WMoveresMode.resize(_, 0, 0, 1, 0)"),
  kpress("D",     "WMoveresMode.resize(_, 0, 0, 0, 1)"),
  ...
  kpress("Shift+W",     "WMoveresMode.resize(_,-1, 0, 0, 0)"),
  kpress("Shift+S",     "WMoveresMode.resize(_, 0,-1, 0, 0)"),
  kpress("Shift+A",     "WMoveresMode.resize(_, 0, 0,-1, 0)"),
  kpress("Shift+D",     "WMoveresMode.resize(_, 0, 0, 0,-1)"),
  ...
  kpress(META.."W",     "WMoveresMode.move(_,-1, 0)"),
  kpress(META.."S",     "WMoveresMode.move(_, 1, 0)"),
  kpress(META.."A",     "WMoveresMode.move(_, 0,-1)"),
  kpress(META.."D",     "WMoveresMode.move(_, 0, 1)"),
})
...
defmenu("mainmenu", {
  ...
  menuentry("Lock screen",    "notioncore.exec_on(_, notioncore.lookup_script('notion-lock'))"),
